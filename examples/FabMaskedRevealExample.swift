...
class FabMaskedRevealExampleViewController: ExampleViewController {

  struct TargetInfo {
    let name: String
    let viewControllerType: UIViewController.Type
    let calculateFrame: CalculateFrame?
  }
  ...
  override func viewDidLoad() {
    ...
    targets.append(.init(name: "Toolbar", viewControllerType: ToolbarViewController.self, calculateFrame: { info in
      return CGRect(x: 0, y: info.containerView.bounds.height - 100, width: info.containerView.bounds.width, height: 100)
    }))
  ...
  func didTapFab(fab: UIView) {
    ...
    transition.calculateFrameOfPresentedViewInContainerView = target.calculateFrame
...
extension FabMaskedRevealExampleViewController: UITableViewDataSource {
...
extension FabMaskedRevealExampleViewController: UITableViewDelegate {
